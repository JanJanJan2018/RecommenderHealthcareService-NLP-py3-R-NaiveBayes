...
```{python}
modalities = pd.read_csv('benefitsContraindications2.csv', encoding = 'unicode_escape') 
```
...
knitr::include_graphics('docTextLength3.png')
```
...
```{python}
print(list(modalities.Document[modalities.length > 4000].index))
print(list(modalities.Topic[modalities.length > 4000]))
...
```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('TopicHist3.png')
```
...
mus
```
The 'Not Professional' class is not getting sorted alphabetically with the others, this could be because it is capitalized. This effects the order of probability readings.
...
def predict_modality(new_review): 
    ...
    if (pr[0][0] == max(pr[0])):
        print('The max probability is Non-Professional services for this recommendation with ', pr[0][0]*100,'%')
        
    elif (pr[0][1] == max(pr[0])):
        print('The max probability is chiropractic therapy for this recommendation with ', pr[0][1]*100,'%')
        
    elif (pr[0][2] == max(pr[0])):
        print('The max probability is cold stone massage for this recommendation with ', pr[0][2]*100,'%')
        
    elif (pr[0][3] == max(pr[0])):
        print('The max probability is cupping therapy for this recommendation with ', pr[0][3]*100,'%')
   
    elif (pr[0][4] == max(pr[0])):
        print('The max probability is massage therapy for this recommendation with ', pr[0][4]*100,'%')
    
    elif (pr[0][5] == max(pr[0])):
        print('The max probability is massage gun therapy for this recommendation with ', pr[0][5]*100,'%')
    
    elif (pr[0][6] == max(pr[0])):
        print('The max probability is mental health therapy for this recommendation with ', pr[0][6]*100,'%')
    
    else:
        print('The max probability is physical therapy for this recommendation with ', pr[0][7]*100,'%')
...
```
In the above user inputs, the one recommending cold stones when the user is cold, should be an error or contraindication, but we haven't made that classification. The data would have to be separated into benefits and contraindications from each document internet searched on each modality. We could do this another time. 
...
```
This user input is more like a review, and the documents other than the 'Non Professional' class were all online articles authored by professionals in the field of each category. The 'Non Professional' category had a 10% recommendation for this user input, just below cupping therapy at 27% and this mental health recommendation with 41%.
...
```{python}
predict_modality('sexy.fun.massage.Asian. pretty girls')
```
The above correctly classify as a non-professional services description. This is great, because when expecting to have legitimate healthcare requests, occasionally some scummy users slip in and want a service that isn't provided. This could also be a recommended use for mental health services for addictions and similar. Although, the mental health documents said this user has a 3% probability compared to the other categories to recommend to this user. The next highest class recommeded was for cold stone massage, which is surprising. The 'pretty' and 'fun' could have been in the cold stone massage ads or when describing the modality and appearance of the marble stones.
...
```{r,error=FALSE, message=FALSE,warning=FALSE}
modalities <- read.csv('benefitsContraindications2.csv',
...
text_df <- tibble(line=1:81, text=modalities1$Document,
...
Moving on, now that we have the non-professional category and our seven categories of legit healthcare recommended categories for users to input. We will need to get a medical services category with more documents, and differentiate between the benefits and contraindications for the other modalities, such as massage therapy, cold stone massage, and cupping therapy. 
